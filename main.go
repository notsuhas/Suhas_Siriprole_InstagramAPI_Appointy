...
// Not the best way to Handle Passwords,
// But Since Hashing is OneWay, slim chances for Reverse Engineering
...
func CreateUserEndpoint(response http.ResponseWriter, request *http.Request) {
	...
	usersCollection := client.Database("Suhas_Siriprole_InstagramDB").Collection("users")
...
func CreatePostEndpoint(response http.ResponseWriter, request *http.Request) {
	...
	postsCollection := client.Database("Suhas_Siriprole_InstagramDB").Collection("posts")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
...
func GetPostsByUserIDEndpoint(response http.ResponseWriter, request *http.Request) {
	...
	// This sets the Number of Results per Page,
	// For Simplicity, Its set as 2, can be bigger
	var perPage int64 = 2

	// This finds the Total Number of Posts that Exist for the given userID
	total, _ := postsCollection.CountDocuments(ctx, filter)

	// This finds the Last Page Number from the Total
